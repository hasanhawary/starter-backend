...
This guide documents all services in the multi-tenant backend that handle core business logic.

## Overview

Services encapsulate business logic and are located in `app/Services/`. They're organized by domain (Auth, Global, Tenant) and provide reusable functionality across controllers.
...
Handles user login with support for email/password and LDAP authentication for central and tenant users.
...
**Location:** `app/Services/Tenant/TenantService.php`

Manages tenant-specific operations and tenant context.
...
9. **Tenant awareness** - Always consider tenant context in multi-tenant services
