...
The Landing Dashboard Kit uses **Laravel Sanctum** for stateless, token-based API authentication with support for traditional login, LDAP, and OTP-based password recovery.
...
```bash
curl -X POST http://starter-backend.test/api/admin/login \
