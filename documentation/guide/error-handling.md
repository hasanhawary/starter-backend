...
description: Exception handling and error responses in multi-tenant environment
---

# Error Handling

This guide documents error handling, custom exceptions, and error response formats in the multi-tenant backend.

## Overview

The multi-tenant backend provides comprehensive error handling with custom exceptions, proper HTTP status codes, and consistent error response formats. All errors are tenant-aware and properly scoped.
...
#### 8. TenantNotFoundException

Thrown when a tenant is not found or invalid.
...
#### 9. SubscriptionExpiredException

Thrown when a tenant's subscription has expired.
...
public function render($request, Throwable $exception)
{
    ...
    if ($request->expectsJson()) {
        return response()->json([
            ...
            'tenant_id' => tenant()?->id, // Include tenant context
...
protected function getStatusCode(Throwable $exception): int
{
    return match (get_class($exception)) {
        ...
        SubscriptionExpiredException::class => 403,
        default => 500,
    };
}
```
...
```php
// Request
GET /api/users (with invalid tenant domain)
...
9. **Include tenant context** - Log and track tenant information in errors
...
- [Multi-Tenancy](/guide/multitenancy/) — Tenant-related errors
