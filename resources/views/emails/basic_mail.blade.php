...
<head>
    ...
    <style>
        body { margin:0; padding:0; background-color: {{ $brand['theme']['secondary'] }}; font-family: 'Segoe UI', Tahoma, sans-serif; }
        a { color: {{ $brand['theme']['primary'] }}; text-decoration:none; }
    </style>
</head>
<body>
            ...
            <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden">
                    ...
                    <td style="padding:24px;background:url('{{ asset($brand['background']) }}') no-repeat center/cover;">
                        <table width="100%">
                            <tr>
                                ...
                                <td align="right" style="color:#fff;font-size:18px;font-weight:600">{{ $brand['name'] }}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
                ...
                <tr>
                    <td style="padding:40px 32px;text-align:center;color:{{ $brand['theme']['text'] }}">
                        ...
                        @if(!empty($data['otp_expires_in']))
                            <p style="font-size:12px;color:#6B7280;">This code will expire in {{ $data['otp_expires_in'] }} minutes.</p>
                        @endif

                        @if(!empty($data['cta']))
                            <a href="{{ $data['cta']['url'] }}" style="display:inline-block;margin-top:24px;padding:12px 32px;background:{{ $brand['theme']['primary'] }};color:#fff;border-radius:8px;font-weight:600">
                                {{ $data['cta']['label'] }}
                            </a>
                        @endif
                ...
                <tr>
                    <td style="padding:24px;text-align:center">
                        ...
                        <span style="font-size:12px;color:{{ $brand['theme']['muted'] }}">{{ now()->toDayDateTimeString() }}</span>
                        <p style="font-size:13px;color:{{ $brand['theme']['muted'] }}">
                            Contact us: <a href="mailto:{{ $brand['contact']['email'] }}">{{ $brand['contact']['email'] }}</a>
                        </p>
