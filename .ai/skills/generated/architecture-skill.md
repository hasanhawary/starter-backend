...
## Purpose
Define the overall project architecture, folder organization, module system, and layering philosophy for this Laravel API backend.

## Philosophy
This is a **service-oriented modular API backend** built on Laravel 13. The architecture follows a strict layer separation: Controllers handle HTTP concerns, Services handle business logic, Models handle data, and Traits provide reusable cross-cutting behavior. The project uses **nwidart/laravel-modules** for optional modular isolation (e.g., Export module). Custom packages (`hasanhawary/*`) extend core functionality (permissions, media, exports, reports, lookups). Every layer has a single responsibility and communicates through well-defined interfaces.
...
- Events use Laravel's broadcasting via Reverb channels
- Jobs implement `ShouldQueue` for async processing
